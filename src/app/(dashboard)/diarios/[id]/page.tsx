  ...
  return (
    <Card className="lg:col-span-2">
      <div className="flex items-center gap-2 mb-3">
        ...
        {!readOnly && (
          <Button size="sm" variant="secondary" onClick={handleSave} disabled={saving} className="ml-auto">
            Salvar
          </Button>
        )}
      </div>
...
export default function DiarioDetalhePage() {
  ...
  const podeAprovar =
    ...
    temPermissao("aprovar_diario");

  const podeExcluir = diario && temPermissao("excluir_diario");
  ...
  const dataFormatada = format(parseISO(diario.data_diario), "dd/MM/yyyy", {
    locale: ptBR,
  });
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {podeAprovar && !showDevolver && (
            <>
              ...
              <Button variant="warning" onClick={() => setShowDevolver(true)}>
                <X className="size-4" />
                Devolver
              </Button>
            </>
        ...
        <Card className="border-[var(--color-warning)]">
          ...
          <div className="flex gap-2">
            <Button
              ...
              disabled={mutationDevolver.isPending || comentarioDevolver.trim().length < 5}
            >
              Confirmar devolução
            </Button>
